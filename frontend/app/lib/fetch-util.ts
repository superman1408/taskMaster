...
const BASE_URL = import.meta.env.VITE_API_URL || "http://locathost:1234/api-v1";
...
api.interceptors.request.use((config) => { 
    ...
    if (token) {
        config.headers.Authorization = `Bearer ${token}`; // Bearer ghfjgshsdfhjshdfkahkshgkjash example
    }
